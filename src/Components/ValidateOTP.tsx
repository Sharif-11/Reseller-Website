...
import { FiArrowLeft, FiKey, FiRefreshCw } from 'react-icons/fi'
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (result.data.alreadySent) {
        setError(result.data.message || 'ওটিপি ইতিমধ্যে পাঠানো হয়েছে')
  ...
  return (
    <div className=' flex items-start justify-center py-16 px-4'>
      <div className='w-full max-w-md bg-white rounded-xl shadow-sm overflow-hidden'>
        ...
        <div className='p-4'>
          {error && (
            <div className='mb-3 p-2 bg-red-50 text-red-600 rounded-md text-xs'>{error}</div>
          )}
