...
export const verifyOrderProducts = async (products: OrderProduct[]) => {
  try {
    const { data } = await axiosInstance.post(`sellers/orders/verify-products`, products)
    const { success, message, statusCode } = data
    const responseData = data?.data
    return {
      success,
...
export const payDue = async ({
  ...
  try {
    const { data } = await axiosInstance.post(`sellers/payments/due`, {
      amount,
      transactionId,
      adminWalletId,
      sellerWalletName,
...
export const getPaymentHistory = async ({
  ...
  try {
    ...
    const statusQueryString = generateQueryString(statusArray)
    const queryParams = statusQueryString ? `${queryString}&${statusQueryString}` : queryString
    const { data } = await axiosInstance.get(`sellers/payments?${queryParams}`)
    const { success, message, statusCode } = data
    const responseData = data?.data
    return {
      success,
...
export const getSellerDashboardStats = async () => {
  try {
    const { data } = await axiosInstance.get(`sellers/dashboard`)
    const { success, message, statusCode } = data
    const responseData = data?.data
    return {
      success,
