...
export const cloudinaryUrl = 'https://api.cloudinary.com/v1_1/dqnw5qaoq/image/upload'
          ...
          onUploadProgress: progressEvent => {
            if (progressEvent.total) {
              ...
              const overallProgress =
                (completedUploads / totalImages) * 90 + (fileProgress * 0.9) / totalImages
  ...
  try {
    ...
    const { data } = await axiosInstance.post(
      ...
      { images: imageUrls },
      {
        onUploadProgress: progressEvent => {
          if (progressEvent.total) {
            const backendProgress = Math.round((progressEvent.loaded * 10) / progressEvent.total)
